(function () {
    ...
    App.Storage = {
        ...
        getSettings:function(){
            return this.get('settings',{
                ...
                aiApiUrl:'https://api.openai.com/v1/chat/completions',aiApiKey:'',aiModel:'gpt-3.5-turbo',
    ...
    App.Questions = {
        ...
        removeBank:function(id){if(!confirm('确定要删除该题库及其所有题目吗？'))return;var banks=App.Storage.getBanks().filter(function(x){return x.id!==id});App.Storage.setBanks(banks);this.render();App.Toast.show('题库已删除','info')},
        ...
        showAIDialog:function(){var body='<div class="form-group"><label>题库名称</label><input type="text" id="inp-ai-bank-name" placeholder="如：数学测试" value="AI生成题库"></div><div class="form-group"><label>题目主题/知识点</label><textarea id="inp-ai-topic" placeholder="请描述要生成的题目主题" rows="3"></textarea></div><div class="form-group"><label>题目数量</label><input type="number" id="inp-ai-count" value="5" min="1" max="20" style="width:80px"></div><div id="ai-status" class="ai-status" style="display:none"></div>';var footer='<button class="btn-secondary" onclick="App.Modal.close()">取消</button><button class="btn-primary" id="btn-ai-gen" onclick="App.Questions.generateAIQuestions()">🤖 生成</button>';App.Modal.open('🤖 AI生成题目',body,footer)},
        generateAIQuestions:function(){var s=App.Storage.getSettings();if(!s.aiApiKey||!s.aiApiUrl){App.Toast.show('请先在设置中配置AI API','warning');return}var topic=document.getElementById('inp-ai-topic').value.trim();if(!topic){App.Toast.show('请输入题目主题','warning');return}var count=parseInt(document.getElementById('inp-ai-count').value)||5;var bankName=document.getElementById('inp-ai-bank-name').value.trim()||'AI生成题库';var statusEl=document.getElementById('ai-status');statusEl.style.display='block';statusEl.className='ai-status processing';statusEl.textContent='⏳ 正在生成题目，请稍候...';document.getElementById('btn-ai-gen').disabled=true;var prompt='请生成'+count+'道关于"'+topic+'"的选择题，每题4个选项(A/B/C/D)，以JSON数组格式返回，格式如下：\n[{"text":"题目","options":{"A":"选项A","B":"选项B","C":"选项C","D":"选项D"},"answer":"B","points":10}]\n只返回JSON数组，不要其他内容。';try{fetch(s.aiApiUrl,{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+s.aiApiKey},body:JSON.stringify({model:s.aiModel||'gpt-3.5-turbo',messages:[{role:'user',content:prompt}],temperature:0.7})}).then(function(res){return res.json()}).then(function(data){var content=data.choices&&data.choices[0]&&data.choices[0].message&&data.choices[0].message.content||'';var jsonMatch=content.match(/\[[\s\S]*\]/);if(!jsonMatch)throw new Error('无法解析AI返回的JSON');var questions=JSON.parse(jsonMatch[0]);var banks=App.Storage.getBanks();var newBank={id:genId(),name:bankName,description:'AI生成 - '+topic,questions:[],createdAt:Date.now()};questions.forEach(function(q){newBank.questions.push({id:genId(),text:q.text,options:q.options,answer:q.answer,points:q.points||10})});banks.push(newBank);App.Storage.setBanks(banks);App.Modal.close();App.Questions.render();App.Toast.show('成功生成 '+questions.length+' 道题目','success')}).catch(function(err){statusEl.className='ai-status error';statusEl.textContent='❌ 生成失败：'+err.message;document.getElementById('btn-ai-gen').disabled=false})}catch(e){statusEl.className='ai-status error';statusEl.textContent='❌ 请求失败：'+e.message;document.getElementById('btn-ai-gen').disabled=false}},
